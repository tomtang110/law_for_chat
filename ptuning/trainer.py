...
if is_sagemaker_mp_enabled():
  ...
  IS_SAGEMAKER_MP_POST_1_10 = version.parse(SMP_VERSION
                                            ) >= version.parse("1.10")
...
class Trainer:
  ...
  def __init__(
    ...
    if (
        self.is_model_parallel or args.deepspeed
    ...
    if ((self.sharded_ddp is not None) or args.deepspeed or
    ...
    if (args.fp16 or args.bf16) and not (
        args.deepspeed or is_sagemaker_mp_enabled() or is_torch_tpu_available()
    ):
    ...
    if (
        is_sagemaker_mp_enabled() and self.use_cuda_amp
        and args.max_grad_norm is not None and args.max_grad_norm > 0
    ):
      raise ValueError(
  ...
  def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
    ...
    if self.args.group_by_length:
      if is_datasets_available(
      ) and isinstance(self.train_dataset, datasets.Dataset):
        lengths = (
  ...
  def get_optimizer_cls_and_kwargs(args: TrainingArguments) -> Tuple[Any, Any]:
    ...
    if args.optim == OptimizerNames.ADAFACTOR:
      ...
      optimizer_kwargs.update(adam_kwargs)
        ...
        OptimizerNames.ADAMW_TORCH, OptimizerNames.ADAMW_TORCH_FUSED
    ]:
      ...
      try:
        ...
        optimizer_kwargs.update({
            "use_kahan_summation":
            strtobool(optim_args.get("use_kahan_summation", "False")),
            "momentum_dtype":
            getattr(torch, optim_args.get("momentum_dtype", "float32")),
            "variance_dtype":
            getattr(torch, optim_args.get("variance_dtype", "float32")),
  ...
  def _report_to_hp_search(
      self, trial: Union["optuna.Trial", Dict[str, Any]], step: int,
  ...
  def train(
    ...
    inner_training_loop = find_executable_batch_size(
        self._inner_training_loop, self._train_batch_size,
  ...
  def _inner_training_loop(
    ...
    for epoch in range(epochs_trained, num_train_epochs):
      ...
      for step, inputs in enumerate(epoch_iterator):
        ...
        if (
            args.logging_nan_inf_filter and not is_torch_tpu_available()
  ...
  def _load_optimizer_and_scheduler(self, checkpoint):
    ...
    checkpoint_file_exists = (
        glob.glob(os.path.join(checkpoint, OPTIMIZER_NAME) +
                  "_*") if is_sagemaker_mp_enabled() else
        ...
        if is_sagemaker_mp_enabled():
          if os.path.isfile(os.path.join(checkpoint, "user_content.pt")):
            ...
            def opt_load_hook(mod, opt):
              opt.load_state_dict(
                  smp.load(
                      os.path.join(checkpoint, OPTIMIZER_NAME), partial=True
                  )
              )
    ...
    if self.use_cuda_amp or self.use_cpu_amp:
      if is_torch_greater_or_equal_than_1_10:
        ctx_manager = (
            ...
            ) if self.use_cpu_amp else torch.cuda.amp.
            autocast(cache_enabled=cache_enabled, dtype=self.amp_dtype)
        )
      else:
  ...
  def compute_loss(self, model, inputs, return_outputs=False):
    ...
    if labels is not None:
      if unwrap_model(model)._get_name(
  ...
  def _save(self, output_dir: Optional[str] = None, state_dict=None):
    ...
    if not isinstance(self.model, PreTrainedModel):
      if isinstance(unwrap_model(self.model), PreTrainedModel):
        ...
        unwrap_model(
            self.model
        ).save_pretrained(output_dir, state_dict=filtered_state_dict)
      else:
        logger.info(
            "Trainer.model is not a `PreTrainedModel`, only saving its state dict."
        )
  ...
  def store_flos(self):
    ...
    if self.args.local_rank != -1:
      self.state.total_flos += (
          distributed_broadcast_scalars([self.current_flos],
  ...
  def _sorted_checkpoints(
    ...
    if self.state.best_model_checkpoint is not None:
      ...
      for i in range(best_model_index, len(checkpoints_sorted) - 2):
        checkpoints_sorted[i], checkpoints_sorted[i + 1] = checkpoints_sorted[
            i + 1], checkpoints_sorted[i]
  ...
  def evaluation_loop(
    ...
    for step, inputs in enumerate(dataloader):
      ...
      if inputs_decode is not None:
        ...
        inputs_host = (
            inputs_decode if inputs_host is None else
            nested_concat(inputs_host, inputs_decode, padding_index=-100)
        )
      ...
      if args.eval_accumulation_steps is not None and (
          step + 1
      ) % args.eval_accumulation_steps == 0:
        ...
        if inputs_host is not None:
          ...
          all_inputs = (
              inputs_decode if all_inputs is None else
              nested_concat(all_inputs, inputs_decode, padding_index=-100)
          )
        if labels_host is not None:
          labels = nested_numpify(labels_host)
          all_labels = (
              labels if all_labels is None else
              nested_concat(all_labels, labels, padding_index=-100)
          )
  ...
  def prediction_loop(
    ...
    inputs_ids = inputs_gatherer.finalize(
